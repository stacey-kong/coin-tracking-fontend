...
function Form(props: FormProps) {
  ...
  const [marketSelectionOpen, setmarketSelectionOpen] =
  ...
  const handleMarketTypeSelectionChange = (selecton: string) => {
    let selectedMarket;

    if (selecton === "Perpetual") {
      selectedMarket = MarketType.PERPETUAL;
    } else {
      selectedMarket = MarketType.SPOT;
    }
    const newstate = { ...formState, marketType: selectedMarket };
    console.log(newstate)
    setmarketSelectionOpen(false);
    setFormState(newstate);
  };
                  ...
                  {marketType.map(function (selection, index) {
                    return (
                      <li
                        ...
                        role="option"
                        onClick={() =>
                          handleMarketTypeSelectionChange(selection)
                        }
                      >
                        <div className="flex items-center">
                          <span className="font-normal ml-3 block truncate">
